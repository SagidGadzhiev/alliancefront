...
const ShoppingItems = ({currency, shoppingProducts}) => {
                        ...
                        <tbody className='wishesItems__table__tbody'>
                        {
                            shoppingProducts.map(i => (
                                <tr className='wishesItems__table__tbody__tr' key={i.id}>
                                    ...
                                    <td className='wishesItems__table__tbody__tr__td' rowSpan={1}>
                                        <input className='wishesItems__table__tbody__tr__td__input' type="number" onChange={(e) => setCount(e.target.value)} value={i.count}/>
                                        <button onClick={() => dispatch(updateCount(countPlus(i.id)))} className='wishesItems__table__tbody__tr__td__btn' type="button">+</button>
                                        <button disabled={i.count <= 1} onClick={() => dispatch(updateCount(countMinus(i.id)))} className='wishesItems__table__tbody__tr__td__btn' type="button">-</button>
                                    </td>
                                    ...
                                    <td className='wishesItems__table__tbody__tr__td' rowSpan={1}>{((i.count) * (i.price)).toFixed(1)}$ - {(i.count) * (i.price * currency).toFixed(0)}сом</td>
                                </tr>
                            ))
                        }
                        </tbody>
